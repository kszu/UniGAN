...
app.config["OUTPUT_FOLDER"] = "static/output/UniGAN_128/samples_testing_2"
...
def unigan():
    ...
    if request.method == "POST" or (request.method == "GET" and img_url):
        ...
        stdout = check_output([python_command], shell=True)
        python_command = "cp {0} {1}".format(os.path.join(app.root_path, app.config["OUTPUT_FOLDER"], "1.jpg"),
                                             os.path.join(app.root_path, app.config["OUTPUT_FOLDER"], output_newname))
        stdout = check_output([python_command], shell=True)

        input = os.path.join(app.config["INPUT_FOLDER"], input_newname)
        output = os.path.join(app.config["OUTPUT_FOLDER"], output_newname)
        ...
        output = os.path.join(app.config["OUTPUT_FOLDER"], "1.jpg")

        return render_template("unigan.html", input=input, output=output, rand_num=np.random.randint(low=1, high=100000, size=1),
                               label1="Original", label2="Unisex", label3="Male", label4="Female", label5="Reconstructed")

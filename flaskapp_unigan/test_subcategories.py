...
def sample_graph():
    ...
    if not os.path.exists(py.join(output_dir, 'generator_unigan_subcategories_only.pb')):
        # model
        ...
        # load freezed model
        with tf.gfile.GFile(py.join(output_dir, 'generator_unigan_subcategories_only.pb'), 'rb') as f:
    ...
    save_dir = '%s/output/%s/samples_testing_subcategories_%s' % (args.flask_path, args.experiment_name, '{:g}'.format(args.test_int))
    py.mkdir(save_dir)
